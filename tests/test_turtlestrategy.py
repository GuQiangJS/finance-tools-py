...
def test_TurtleStrategy_1():
    ...
    assert 0==ts.on_calc_sell_amount(datetime.date(2000,1,1),symbol,10.2,1000,0,10)
    assert not ts.on_check_sell(datetime.date(2000,1,1),symbol,10.2,1000,0,10)
    assert 1==len(ts.holds[symbol])
    # 当前时间不包含在卖出点字典中-开始
...
def test_TurtleStrategy_2():
    ...
    assert not ts.on_check_sell(datetime.date(2000,1,1),symbol,10.2,1000,100,10)
    assert 100==ts.on_calc_sell_amount(datetime.date(2000,1,1),symbol,10.2,1000,100,10)
...
def test_TurtleStrategy_3():
    ...
    assert not ts.on_check_sell(datetime.date(2000,1,1),symbol,10.2,1000,0,10)
    # 没有到达止损点位
    assert 0==ts.on_calc_sell_amount(datetime.date(2000,1,1),symbol,9.01,1000,0,10)
    # 到达止损点位
    assert 100==ts.on_calc_sell_amount(datetime.date(2000,1,1),symbol,9,1000,0,10)
    assert 0==len(ts.holds[symbol])
...
def test_TurtleStrategy_4():
    ...
    assert not ts.on_check_sell(datetime.date(2000, 1, 1), symbol, 10.2, 1000,
                                0, 10)
    # 没有到达止盈点位
    assert 0 == ts.on_calc_sell_amount(datetime.date(2000, 1, 1), symbol, 10.99,
                                       1000, 0, 10)
    # 到达止盈点位
...
def test_TurtleStrategy_4():
    ...
    ts = TurtleStrategy('', holds=holds,buy_dict={symbol:[datetime.date(2000,1,1)]})

    assert 1 == len(ts.holds[symbol])
    # 没有到达加仓点位
    assert not ts.on_check_buy(datetime.date(2000, 1, 1), symbol, 10.49, 1000,verbose=2)
    # 到达加仓点位
    assert ts.on_check_buy(datetime.date(2000, 1, 1), symbol, 10.50, 1000,verbose=2)
    # 没有加仓点位
    assert 0 == ts.on_calc_buy_amount(datetime.date(2000, 1, 1), symbol, 10.49, 1000,verbose=2)
    # 到达购买点，但是资金不足
    assert 0 == ts.on_calc_buy_amount(datetime.date(2000, 1, 1), symbol, 10.50, 1000,verbose=2)
    # 到达购买点，资金充足
    assert 100 == ts.on_calc_buy_amount(datetime.date(2000, 1, 1), symbol, 10.50, 1200,verbose=2)
    ...
    assert 100 == ts.on_calc_buy_amount(datetime.date(2000, 1, 1), symbol, 10.50, 1200,verbose=2)
    # 到达加仓点位。但是到达持仓限制
    assert not ts.on_check_buy(datetime.date(2000, 1, 1), symbol, 10.50, 1000,verbose=2)
...
def test_TurtleStrategy_5():
    ...
    assert 100==ts.on_calc_buy_amount(datetime.date(2000,1,1),symbol,20,10000)
    assert 2 == len(ts.holds[symbol])
    ...
    assert ts.on_check_sell(datetime.date(2000,1,2),symbol,15,9999,300,-1)
    assert ts.on_check_sell(datetime.date(2000,1,2),symbol,15,9999,-1,-1)

    assert 200==ts.on_calc_sell_amount(datetime.date(2000,1,2),symbol,15,9999,-1,-1)
    assert 0==len(ts.holds[symbol])