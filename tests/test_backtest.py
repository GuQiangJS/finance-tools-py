...
def test_backtest_calc(init_global_data):
    bt = BackTest(pytest.global_data, callbacks=[AHundredChecker(
...
def test_backtest_calc_mutil(init_global_data):
    ...
    bt = BackTest(data, callbacks=[AHundredChecker(
...
def test_backtest_holdprice(init_global_data):
    ...
    bt = BackTest(data, init_cash=100000, callbacks=[AHundredChecker(
        buy_dict={'000001': [dt(1999, 1, 1), dt(2001, 1, 1)],
...
def test_example():
    ...
    assert np.round(bt.total_assets_cur, 2) == np.round(bt.available_cash + (6.7) * 100, 2)
...
def test_allin():
    ...
    bt = BackTest(data, init_cash=5000, callbacks=[AllInChecker(
        buy_dict={'000001': [dt(1998, 1, 1), dt(2000, 1, 1)]},
        sell_dict={'000001': [dt(1999, 1, 1)]})])
    bt.calc_trade_history()
    print(bt.report())
    ...
    assert np.round(6.7, 2) == np.round(bt.hold_price_cur['000001'][0], 2)
    assert np.round(bt.total_assets_cur, 2) == np.round(bt.available_cash + (6.7) * 1200, 2)
